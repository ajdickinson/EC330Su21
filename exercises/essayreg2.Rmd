...
The mean of the response `y` `r direct` with increasing `x1`. Variable `x2` has no
influence on the response since the estimated confidence intervals contain zero.
...
The R-squared is `r signif(rsq, 4)` and thus the variance explained by the regression is `r fmt(100 * rsq, 2)`.
