...
.hi-blue[Questions]:
  ...
  - Why do cities grow beyond .orange[one factory]?
 
--
 
  - Why are there .purple[differences in size] across cities?
 
  - .orange[Where] do cities emerge?
...
Where to start? .hi-blue[Axoim 5]

--

.center[.orange[Axiom 5: _Competition generates zero economic profit_]]
...
Similar firms .hi-blue[share inputs] to benefit from economies of scale<sup>.hi[†]</sup>

.footnote[
.hi[†] Economies of scale: _More output_ $\rightarrow$ _smaller cost per unit_
]
...
Continuing the example in tech, consider a labor pool of software programmers
...
- Firms and workers are .orange[not always perfectly matched]
...
--
  ...
  - I like to think that this classroom has knowledge spillovers 
...
# (iv). Labor pool sharing

When firms fail, where do the workers move?
...
- Evidence of higher .hi.purple[rates of entry]
  ...
  - More firms are born where .orange[output is higher]; that is, where  the industry is .purple[clustered] (Carlton, 1986)
...
.qa[A1]: In words, this equation says:
  ...
  - .orange[A few] cities will be pretty big
...
- Inadequate .orange[transportation infastructure] elsewhere
...
.footnote[
.orange[†] I don't know. Maybe somebody does. But you definitely can't say from the 45% number. Much of modern econ is about figuring out when relationships _are_ causal. For a completely unrelated, but informative and entertaining example, see [this video](https://www.youtube.com/watch?v=6YrIDhaUQOE&t=133s). 
]
...
2) .hi.green[Pollution] increases
  ...
  - More .orange[workers] $\implies$ more .purple[production] $\implies$ more .green[pollution]?
...
1) .purple[Higher] levels of utility are .orange[preferred] to .purple[lower] levels. And more consumption is better than less

2) Utility is _ordinal, not cardinal_ meaning only the .hi.orange[rank] of the number matters, .hi.purple[not the level]
...
.hi-blue[Locational Equilibrium] occurs when utility levels (valuations) across cities are the same for all workers


--

  - In practice, we usually do this by .orange[worker type] (demographic, income level, education, etc)
...
--
 ...
 More on this later in the term (.orange[place-based] policies). 
...
.hi.purple[Urban Economics]: .hi-blue[Economic Growth] is an increase in the _utility level_ of a typical resident
...
1) Increases in natural resources (.orange[gold is found under a city])

--

2) Increases in physical capital (.orange[computers] `r emo::ji("computer")`)

--

--

3) Increases in human capital (.orange[education] `r emo::ji("mortar_board")`)

--

--

4) Technological progress (.orange[computers invented])
...
- .hi-blue[Note]: We rested on

 - the implicit assumption that .orange[people are perfectly mobile] (and they are the same)
...
- .hi[Innovation] (tech progress) in one city .orange[shifts utility per worker] curve up
...
- Eventually, a .orange[new equilibrium] is reached where .hi[utility per worker] is the .hi[same across both cities]
...
.hi-blue[Note]: If there is an .hi.orange[innovation for the entire economy], then:

  - .hi[Both cities] experience .orange[upward shift] of utility curve
  
  - No utility gap at original populations, so .orange[no migration]
